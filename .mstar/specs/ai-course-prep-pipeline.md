...
6. **AI 标注合同**：`stage_plan[]`、`blocks[]` 与 `review_schedule`（含其 `plans[]` 每一档）每一项均含
   ...
   **第 4 类（B4 追加，不新增 AC 编号）**：答案分布偏置 —— 单选项任一字母占比 **> 40%**、
   判断类题同类真值 **> 80%** 时失败关闭，样本量 **< 10** 时跳过占比判定但必须输出 `insufficient-sample`
   （**不静默通过**）。作用域 = **只对新生成课失败关闭**；B4 之前已上线的既有课
   （`15040` / `15043` / `15044` / `00898` / `02333`）只报告（写明实测偏置 + `R15`/`R33` 引用），
   不阻断闸门。判据、解析口径与作用域信号的**实现细节**见
   `.mstar/iterations/jiangsu-ai-course-pipeline-b4-2026-09-18/specs/design-notes.md`
   （**迭代级**设计说明）；本项为规范性条款，其成立**不依赖**该文档。
...
**B4+ 启动门槛的进度留痕（口径 + 实测值）**：门槛② 的进度值 = **AI 备考层覆盖课数**
（可数口径 = `evidence.json` 中 `eligibility.level == "L1"` **且**已产出
`sources/jiangsu/courses/<code>/content.json` 的课数）。**实测（B3 收口，2026-09-13）= 5 门**
（`15040` / `15043` / `15044` / `00898` / `02333`）。
迭代 `jiangsu-ai-course-pipeline-b4-2026-09-18` 的目标是把该值推到 **7 门**（+`04747` / `04751`）——
**它是目标而非实测值**，其**实测值在 iteration-close 回写**（该迭代 compass § 切片规模判断留有
缩减为 6 门的路径，届时以回写值为准）。门槛值 **≥ 10 不变**，本行只记录进度与口径。
（**注意**：本行的「B4+ 门槛」指上表 **B4 批次**（其余专业分批）的启动门槛，
与迭代 id `jiangsu-ai-course-pipeline-b4-2026-09-18` 不是同一件事。）
...
>
> **B2a 收敛记录（2026-09-13）**：O2 与 O4 均已收敛（见下表）；O1 与 O3 仍开放，分别阻塞实践课生成与导出/交互批次。
...
| **O2 真题授权** | 官方公开真题原件的版权边界与「可转载题文」范围 | 无 | **已收敛（2026-09-12，用户决定）**：官方公开真题原件**可转载，但须登记来源**。B2 实际未使用 —— 实测仓内**无任何真题原件**（`sources/**` 无 papers），本轮零影响。**仍阻塞**首次引入真题时（须先落来源登记） |
| **O3 导出与交互** | Anki / CSV 导出与站内自测的批次位置（`D3` 的后续） | 无 | 不阻塞 B1–B4 的核心交付；归属批次由后续 clarify 决定 |
| **O4 LLM 预算** | 批量生成的费用上限与失败重试策略 | 无 | **已收敛（2026-09-12，用户决定）**：无 API key → 用 `agent` 后端（无外部费用）；以 B1 实测 1473 tokens/点折算硬上限。B2a 实测：`15043` 产出 **568,942 字符（355,589 tokens）**，低于**未放宽**的 601,600 上限，故原推导虽不严谨但上限本身未被突破 |
