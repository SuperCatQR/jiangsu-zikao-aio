...
**Explicit non-goals:** no 13017/13000/13003/13180 work; no syllabus body prose, code, screenshots, question text, exam-period or applicability inference; no textbook, ISBN, physical-book, true-paper, reviewer, `reviewed`, or `publishable` claim; no edits outside the two course write surfaces and permitted focused tests. During Execute, the user explicitly authorized one minimal baseline-contract repair in `tests/test_site_integration.py` for five stale assertions against already-current hub wording, plus the QC3-authorized one-line addition of `04747` to the generic `FIVE_KIND_CODES` coverage set and the subsequent page-shape normalization of `content/jiangsu/courses/04747/index.md`; the hub remains read-only.
...
- User authorization recorded during Execute permits the 04747 plan to update only the five stale expected strings in `tests/test_site_integration.py` that no longer match the already-current `content/jiangsu/courses/index.md` headings/text.
- QC3 S-001 user authorization additionally permits one test-only coverage change in that same file: add `04747` to `FIVE_KIND_CODES` so the generic navigation/start-here contract exercises the selected course.
- The permitted file is limited to the two affected test functions, their five stale literals, and the one explicit `FIVE_KIND_CODES` tuple addition: the shared test remains a read-only product/hub observer; no hub, other shared tests, scripts, or source artifacts may change.
### QC S-001 follow-up — 04747 page-shape normalization

- The user selected the minimal product-side resolution after verification showed that the generic site-integration parser expects a cross-link bar and four bare ordered Markdown links, while the 04747 page currently uses the 02333/00898 table-style start-here block.
- The permitted fix is limited to `content/jiangsu/courses/04747/index.md`: add a self-link-free five-page cross-link bar and expose the same four existing reader-path destinations as exact ordered Markdown link lines that the generic contract parses.
- Preserve the existing four action descriptions, official-source wording, evidence gaps, lifecycle ceiling, and all other page content. Do not modify `content/jiangsu/courses/index.md`, `tests/test_site_integration.py` beyond the already committed S-001 tuple addition and five literals, any other course page, source artifact, script, schema, or test logic.
- This remains the 04747 business plan's navigation-shape repair; no third plan or new evidence claim is introduced. The fix requires fresh implementation evidence, qc3 targeted re-review in the existing `qc3.md`, and mandatory QA on the final aligned branch.
