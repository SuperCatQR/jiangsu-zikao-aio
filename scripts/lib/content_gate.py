"""Content copyright/status/code field gate (formerly validate-content core)."""
...
ALLOWED_COMPLETENESS = set(COMPLETENESS)
ALLOWED_LIFECYCLE = set(LIFECYCLES)
DATE_RE = re.compile(r"\b\d{4}-\d{2}-\d{2}\b")
COURSE_CODE_RE = re.compile(r"^[0-9]{5}$")
MAJOR_CODE_RE = re.compile(r"^[0-9]{6}[A-Z]?$")
RAW_PRIVATE_RE = re.compile(
    r"https://raw\.githubusercontent\.com/[^\s)]+/(zikao-materials|.*private)", re.I
)
GITHUB_PRIVATE_BLOB_RE = re.compile(
    r"https://github\.com/[^\s)]+/zikao-materials/(?:blob|raw)/", re.I
)
...
def validate_pdf_manifest(root: Path, path: Path | None = None) -> list[str]:
    ...
    errors: list[str] = []
    if not path.exists():
        return [f"{rel_label(root, path)}: 缺 PDF 处理 manifest"]
    rows = list(csv.DictReader(path.open(encoding="utf-8-sig")))
    fields = set(rows[0].keys()) if rows else set()
    missing = MANIFEST_REQUIRED - fields
    if missing:
        errors.append(f"{rel_label(root, path)}: manifest 缺字段：{', '.join(sorted(missing))}")
    for i, row in enumerate(rows, 2):
        for key in ["source_pdf", "raw_xml", "raw_txt", "raw_view_html", "extracted_md"]:
            rel = row.get(key, "")
            if not rel:
                errors.append(f"{rel_label(root, path)}:{i}: {key} 为空")
                continue
        ...
        if not row.get("extraction_policy"):
            errors.append(f"{rel_label(root, path)}:{i}: extraction_policy 为空")
    return errors


def run_content_gate(root: Path) -> list[str]:
    content = root / "content" / "jiangsu"
    errors: list[str] = []
    for md in content.rglob("*.md"):
        if md.name in {"plan.extracted.md", "plan.pipeline-notes.md"}:
            continue
        errors.extend(validate_file(root, md))
    errors.extend(validate_pdf_manifest(root))
    return errors
