...
`04747` 已进入 AI 备考流水线：`plan.md` / `practice.md` / `review.md` / `knowledge/*.md` 由
...
def _without_whitespace(title: str) -> str:
    ...
    知识模型逐字照录考纲抽取件（`04747` 抽取件印 `第 6 章`），手写 `syllabus.md` 的章目索引表把章号
    内部空白归一成 `第6章` —— 两侧的章集合 / 章序 / 章名完全相同，差异**只在空白**（W4.5 / Task 0b）。
    这是本模块比较侧的**唯一**放宽：空白以外（大小写、标点、全半角、数字）仍须逐字相同，章数与章序
    仍由列表比较承担。同一课的 `evidence_gate` 已按此规则判定模型与页面一致，本模块不得比它更严。
    """
...
def _chapter_page_titles() -> list[str]:
    ...
    titles = []
    for index in range(1, len(CHAPTER_NAMES_04747) + 1):
...
def test_04747_plan_sequence_matches_syllabus_index_order():
    ...
    手写时代该表名为 `## 建议节奏：按章节顺序执行`、列为 `顺序 / 章节（标题照录）/ 类型`；`04747`
    ...
    assert [_without_whitespace(name) for name in plan_names] == [
        _without_whitespace(name) for name in CHAPTER_NAMES_04747
    ]
...
def test_04747_chapter_assessed_and_non_assessed_partition():
...
def test_04747_six_pages_reachable_from_course_index():
